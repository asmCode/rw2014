...
void InstanceTest::Initialize()
{
	...
	m_triangles[1].Transform = sm::Matrix::TranslateMatrix(10, 0, 0);
	m_triangles[1].Color.Set(0, 0, 1, 1);

	m_triangles[2].Transform = sm::Matrix::TranslateMatrix(20, 0, 0);
	m_triangles[2].Color.Set(0, 1, 0, 1);
...
void InstanceTest::SetMaterial(Material* material)
{
	m_material = material;
}

Material* InstanceTest::GetMaterial() const
{
	return m_material;
}

void InstanceTest::Apply()
{
	/*
	glBindBuffer(GL_ARRAY_BUFFER, m_vertexDataBufferId);
	...
	*/
	...
	for (int i = 0; i < 4; i++)
	{
		int a = sizeof(sm::Vec4);
		int b = sizeof(TriangleDataColor);

		glEnableVertexAttribArray(baseVertexDataIndex + i);
...
void InstanceTest::Draw()
{
	...
	//glDrawElementsInstancedBaseVertex(GL_TRIANGLES, 3, GL_UNSIGNED_SHORT, 0, 1, 0);
...
TriangleDataColor* InstanceTest::GetTrianglesData() const
{
	return m_triangles;
}

uint32_t InstanceTest::GetTrianglesCount() const
{

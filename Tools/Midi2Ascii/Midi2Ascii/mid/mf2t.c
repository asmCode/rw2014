...
int wmain(int argc, wchar_t **argv)
{
    ...
    Mf_nomerge = 1;
    while ((c = wgetopt(argc, argv, L"mnbtvf:")) != -1) {
        switch (c) {
            case L'm':
                Mf_nomerge = 0;
                break;
    ...
    if (optind < argc) {
        if (! _wfreopen(argv[optind++], L"rb", stdin)) {
            perror("_wfreopen");
            exit(1);
        }
    } else _setmode(_fileno(stdin), O_BINARY);

    if (optind < argc && !_wfreopen(argv[optind], L"w", stdout)) {
        perror("_wfreopen");
        exit(1);
    }
